...
pub enum Error {
    ...
    #[error("io error: {} with path: {:?}", .io, .path)]
    IO { io: io::Error, path: PathBuf },
...
pub async fn get_cert_for_host_key(
    ...
    let mut cert_config_path = config.prefix.join(host);
    cert_config_path.push(key + ".json");
    cert_config_path = cert_config_path.canonicalize().map_err(|err| Error::IO {
        io: err,
        path: cert_config_path.clone(),
    })?;
    ...
    let mut cert_config_file = File::open(&cert_config_path)
        .await
        .map_err(|err| Error::IO {
            io: err,
            path: cert_config_path.clone(),
    ...
    let mut ca_key_file = File::open(&ca_key_path).await.map_err(|err| Error::IO {
        io: err,
        path: ca_key_path.clone(),
    })?;
    let mut ca_key_string = String::new();
    ca_key_file
        .read_to_string(&mut ca_key_string)
        .await
        .map_err(|err| Error::IO {
            io: err,
            path: ca_key_path.clone(),
